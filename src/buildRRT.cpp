...
void rrtLoop(emxArray_real_T *T, const double jointLimits[20], const struct0_T
             *kC, double panHeight, const double U[10], double Dt, double dt,
             double *nodeIDCount, const double nGoal[13], const double uBDot[6],
             int legNum)
{
  double alphaRand;
  double xMax;
  ...
  if (b_xi == 0) {
    memcpy(&b_xRand[0], &nGoal[0], 13U * sizeof(double));
  }
  ...
  for (i = 0; i < 5; i++) {
    ...
    uB[0] = ((((kC->l2 + kC->l3 * cos(-b_xRand[4])) + kC->l4 * cos(kC->zeta)) +
              kC->l5 * cos(b_xRand[5] + kC->zeta)) - kC->l7) * cos(b_xRand[3]);
    uB[1] = ((((kC->l2 + kC->l3 * cos(-b_xRand[4])) + kC->l4 * cos(kC->zeta)) +
              kC->l5 * cos(b_xRand[5] + kC->zeta)) - kC->l7) * sin(b_xRand[3]);
    ...
    distance_data[i] = 0.5 * norm(xRand) + 0.5 * (double)xi;
  ...
  if (xi < 5) {
    while (xi + 1 <= 5) {
      if (distance_data[xi] < xMax) {
        xMax = distance_data[xi];
        itmp = xi;
      }
  ...
  for (i9 = 0; i9 < 13; i9++) {
    T->data[xi + T->size[0] * i9] = xNew_data[i9];
  }

  for (i9 = 0; i9 <= loop_ub; i9++) {
    T->data[xi + T->size[0] * (i9 + 13)] = candTransArrays->data[itmp +
