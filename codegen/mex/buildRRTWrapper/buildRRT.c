...
static void rrtLoop(const emlrtStack *sp, emxArray_real_T *T, const real_T
                    ...
                    *nodeIDCount, const real_T uBDot[6], int32_T legNum);
                    ...
                    *nodeIDCount, const real_T uBDot[6], int32_T legNum)
{
  ...
  emlrtDynamicBoundsCheckFastR2012b(i, 1, 2000, &lb_emlrtBCI, sp);
  loop_ub = T->size[1];
  i = r3->size[0];
  r3->size[0] = loop_ub;
  ...
  emxEnsureCapacity(sp, (emxArray__common *)r4, i, (int32_T)sizeof(real_T),
                    &j_emlrtRTEI);
  for (i = 0; i < 13; i++) {
    r4->data[r4->size[0] * i] = xNew_data[xNew_size[0] * i];
  }

  loop_ub = unusedU4->size[1];
  for (i = 0; i < loop_ub; i++) {
    r4->data[r4->size[0] * (i + 13)] = unusedU4->data[unusedU4->size[0] * i];
  }
  ...
  loop_ub = r4->size[1];
  for (i = 0; i < loop_ub; i++) {
    T->data[(b_nodeIDCount + T->size[0] * r3->data[i]) - 1] = r4->data[r4->size
      [0] * i];
  }
              ...
              *path)
{
  ...
  for (absb = 0; absb < 1999; absb++) {
    ...
    rrtLoop(&st, T, jointLimits, kC, panHeight, U, Dt, dt,
            &transitionArrayLength, uBDot, legNum);
