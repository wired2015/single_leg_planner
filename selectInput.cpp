                 ...
                 *transitionArray)
{
  ...
  i3 = candStates->size[0] * candStates->size[1];
  candStates->size[0] = U_SIZE;
  candStates->size[1] = NODE_SIZE;
  emxEnsureCapacity((emxArray__common *)candStates, i3, (int)sizeof(double));
  n = U_SIZE * NODE_SIZE;
  for (i3 = 0; i3 < n; i3++) {
    candStates->data[i3] = 0.0;
  }
  ...
  i3 = distance->size[0] * distance->size[1];
  distance->size[0] = 1;
  distance->size[1] = U_SIZE;
  emxEnsureCapacity((emxArray__common *)distance, i3, (int)sizeof(double));
  for (i3 = 0; i3 < U_SIZE; i3++) {
    distance->data[i3] = 0.0;
  }

  emxInit_real_T(&U_joint, 2);
  i3 = U_joint->size[0] * U_joint->size[1];
  U_joint->size[0] = U_SIZE;
  U_joint->size[1] = 3;
  emxEnsureCapacity((emxArray__common *)U_joint, i3, (int)sizeof(double));
  n = U_SIZE * 3;
  for (i3 = 0; i3 < n; i3++) {
    U_joint->data[i3] = 0.0;
  }
  ...
  for (i = 0; i + 1 <= U_SIZE; i++) {
    for (i3 = 0; i3 < 2; i3++) {
      U_joint->data[i + U_joint->size[0] * i3] = U[i + 5 * i3];
    }
  ...
  while (i + 1 <= U_SIZE) {
    ...
    for (i3 = 0; i3 < 3; i3++) {
      u[i3] = U_joint->data[i + U_joint->size[0] * i3];
    }
    ...
    xInit->size[0] = 1;
    xInit->size[1] = (ix - i3) + 1;
    emxEnsureCapacity((emxArray__common *)xInit, ixstart, (int)sizeof(double));
    n = ix - i3;
    for (ix = 0; ix <= n; ix++) {
      xInit->data[xInit->size[0] * ix] = xNear->data[i3 + ix];
    }

    // xInitOrig = xInit;
    i3 = r2->size[0] * r2->size[1];
    r2->size[0] = 1;
    r2->size[1] = (int)((numIterations + 1.0) * 6.0);
    ...
    for (ixstart = 0; ixstart < (int)numIterations; ixstart++) {
      ...
      beta = dt / 2.0;
      for (i3 = 0; i3 < 6; i3++) {
        b_xInit[i3] = xInit->data[i3] + beta * k1[i3];
      }
      ...
      beta = dt / 2.0;
      for (i3 = 0; i3 < 6; i3++) {
        b_xInit[i3] = xInit->data[i3] + beta * k2[i3];
      }
      ...
      absxk = dt / 6.0;
      for (i3 = 0; i3 < 6; i3++) {
        b_xInit[i3] = xInit->data[i3] + beta * k3[i3];
      }

      for (i3 = 0; i3 < 3; i3++) {
        c_xInit[i3] = b_xInit[3 + i3];
      }
      ...
      emxEnsureCapacity((emxArray__common *)r1, i3, (int)sizeof(double));
      for (i3 = 0; i3 < 6; i3++) {
        r1->data[r1->size[0] * i3] = xInit->data[i3] + absxk * (((k1[i3] + 2.0 *
          k2[i3]) + 2.0 * k3[i3]) + c_xInit[i3]);
      }
      ...
      i3 = xInit->size[0] * xInit->size[1];
      xInit->size[0] = 1;
      xInit->size[1] = r1->size[1];
      emxEnsureCapacity((emxArray__common *)xInit, i3, (int)sizeof(double));
      n = r1->size[0] * r1->size[1];
      for (i3 = 0; i3 < n; i3++) {
        xInit->data[i3] = r1->data[i3];
      }
      ...
      n = r1->size[1];
      for (ix = 0; ix < n; ix++) {
        r2->data[i3 + ix] = r1->data[r1->size[0] * ix];
      }
    }
    ...
    r3->size[0] = 1;
    r3->size[1] = 5 + r1->size[1];
    emxEnsureCapacity((emxArray__common *)r3, i3, (int)sizeof(double));
    for (i3 = 0; i3 < 3; i3++) {
      r3->data[r3->size[0] * i3] = 0.0;
    }

    n = r1->size[1];
    for (i3 = 0; i3 < n; i3++) {
      r3->data[r3->size[0] * (i3 + 3)] = r1->data[r1->size[0] * i3];
    }

    r3->data[r3->size[0] * (3 + r1->size[1])] = 0.0;
    r3->data[r3->size[0] * (4 + r1->size[1])] = 0.0;
    i3 = r1->size[0] * r1->size[1];
    r1->size[0] = 1;
    r1->size[1] = r3->size[1];
    emxEnsureCapacity((emxArray__common *)r1, i3, (int)sizeof(double));
    n = r3->size[1];
    for (i3 = 0; i3 < n; i3++) {
      r1->data[r1->size[0] * i3] = r3->data[r3->size[0] * i3];
    }

    n = r1->size[1];
    for (i3 = 0; i3 < n; i3++) {
      candStates->data[i + candStates->size[0] * i3] = r1->data[r1->size[0] * i3];
    }

    n = r2->size[1];
    for (i3 = 0; i3 < n; i3++) {
      candTransArrays->data[i + candTransArrays->size[0] * i3] = r2->data
        [r2->size[0] * i3];
    }
    ...
    // state.
    n = candStates->size[1];
    i3 = b_candStates->size[0] * b_candStates->size[1];
    b_candStates->size[0] = 1;
    b_candStates->size[1] = n;
    emxEnsureCapacity((emxArray__common *)b_candStates, i3, (int)sizeof(double));
    for (i3 = 0; i3 < n; i3++) {
      b_candStates->data[b_candStates->size[0] * i3] = candStates->data[i +
        candStates->size[0] * i3];
    }
    ...
    distance->data[i] = (1.0 - aGain) * hDiff + aGain * (double)i4;
  ...
  n = candStates->size[1];
  i3 = xNew->size[0] * xNew->size[1];
  xNew->size[0] = 1;
  xNew->size[1] = n;
  emxEnsureCapacity((emxArray__common *)xNew, i3, (int)sizeof(double));
  for (i3 = 0; i3 < n; i3++) {
    xNew->data[xNew->size[0] * i3] = candStates->data[i + candStates->size[0] *
      i3];
  }
  ...
  for (i3 = 0; i3 < n; i3++) {
    transitionArray->data[transitionArray->size[0] * i3] = candTransArrays->
      data[i + candTransArrays->size[0] * i3];
  }
