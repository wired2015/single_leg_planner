  ...
  emxArray_real_T *states)
{
  ...
  i20 = states->size[0] * states->size[1];
  states->size[0] = (int32_T)emlrtNonNegativeCheckFastR2012b(N, &d_emlrtDCI, sp);
  states->size[1] = 6;
  emxEnsureCapacity(sp, (emxArray__common *)states, i20, (int32_T)sizeof(real_T),
                    &p_emlrtRTEI);
  ...
  while (b_i <= N) {
    ...
    dv20[3] = jointLimits[0] + (jointLimits[1] - jointLimits[0]) * r;
    dv20[4] = u1[1];
    dv20[5] = u1[2];
    dv20[6] = 0.0;
    dv20[7] = 0.0;
    dv20[8] = (jointLimits[9] - jointLimits[8]) * b_r + jointLimits[8];
    dv20[9] = (jointLimits[11] - jointLimits[10]) * xMax + jointLimits[10];
    dv20[10] = (jointLimits[13] - jointLimits[12]) * xMin + jointLimits[12];
    dv20[11] = 0.0;
    dv20[12] = 0.0;
    for (i20 = 0; i20 < 3; i20++) {
      u1[i20] = dv20[3 + i20];
    ...
    for (i20 = 0; i20 < 3; i20++) {
      xMax = 0.0;
      for (i = 0; i < 3; i++) {
        xMax += TP2B[i20 + (i << 2)] * uP[i];
      }

      u1[i20] = xMax + TP2B[12 + i20];
    }

    for (i20 = 0; i20 < 3; i20++) {
      states->data[(b_i + states->size[0] * i20) - 1] = u1[i20];
    }
