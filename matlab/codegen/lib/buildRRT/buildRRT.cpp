...
static void rrtLoop(emxArray_real_T *T, const double jointRange[6], const double
...
// Return Type  : void
//
void buildRRT(const double nInit[11], const double nGoal[11], int NUM_NODES,
              const double jointLimits[12], double K, const double HGAINS[3],
              int NODE_SIZE, const double U[10], int U_SIZE, double dt, double
              Dt, const double kinematicConst[12], double ankleThreshold,
              boolean_T exhaustive, double threshold, int goalSeedFreq,
              emxArray_real_T *T, emxArray_real_T *path)
{
  ...
  T->size[1] = i0;
  emxEnsureCapacity((emxArray__common *)T, i, (int)sizeof(double));
  dist = rt_roundd_snf((double)NODE_SIZE + transitionArrayLength);
  if (dist < 2.147483648E+9) {
    if (dist >= -2.147483648E+9) {
      i = (int)dist;
  ...
  if (!exhaustive) {
    ...
    while ((dist > threshold) && (transitionArrayLength < NUM_NODES)) {
      rrtLoop(T, jointRange, jointLimits, kinematicConst, K, U, Dt, dt,
              NODE_SIZE, U_SIZE, HGAINS, ankleThreshold, &transitionArrayLength,
              nGoal, goalSeedFreq);
      nearestNeighbour(nGoal, T, kinematicConst, transitionArrayLength,
                       NODE_SIZE, unusedU0, next, &dist);
    }
  }

  // Find the closest node in the tree to the goal node.
  nearestNeighbour(nGoal, T, kinematicConst, transitionArrayLength, NODE_SIZE,
                   next, unusedU0, &unusedU2);
  transitionArrayLength = next->data[0];
  i = path->size[0] * path->size[1];
  path->size[0] = 1;
  path->size[1] = next->size[1] + unusedU0->size[1];
  emxEnsureCapacity((emxArray__common *)path, i, (int)sizeof(double));
  loop_ub = next->size[1];
  for (i = 0; i < loop_ub; i++) {
    path->data[path->size[0] * i] = next->data[next->size[0] * i];
  }

  loop_ub = unusedU0->size[1];
  for (i = 0; i < loop_ub; i++) {
    path->data[path->size[0] * (i + next->size[1])] = unusedU0->data
      [unusedU0->size[0] * i];
  }
  ...
  while ((transitionArrayLength != 0.0) && (next->data[1] != 0.0)) {
    ...
    loop_ub = T->size[1];
    i = next->size[0] * next->size[1];
    next->size[0] = 1;
    next->size[1] = loop_ub;
    emxEnsureCapacity((emxArray__common *)next, i, (int)sizeof(double));
    for (i = 0; i < loop_ub; i++) {
      next->data[next->size[0] * i] = T->data[((int)transitionArrayLength +
        T->size[0] * i) - 1];
    }
    ...
    emxEnsureCapacity((emxArray__common *)b_path, i, (int)sizeof(double));
    loop_ub = path->size[1];
    for (i = 0; i < loop_ub; i++) {
      b_loop_ub = path->size[0];
      for (i0 = 0; i0 < b_loop_ub; i0++) {
        b_path->data[i0 + b_path->size[0] * i] = path->data[i0 + path->size[0] *
          i];
      }
    }
    ...
    loop_ub = next->size[1];
    for (i = 0; i < loop_ub; i++) {
      b_path->data[path->size[0] + b_path->size[0] * i] = next->data[next->size
        [0] * i];
    }
    ...
    for (i = 0; i < loop_ub; i++) {
      ...
      for (i0 = 0; i0 < b_loop_ub; i0++) {
        path->data[i0 + path->size[0] * i] = b_path->data[i0 + b_path->size[0] *
          i];
      }
    }
