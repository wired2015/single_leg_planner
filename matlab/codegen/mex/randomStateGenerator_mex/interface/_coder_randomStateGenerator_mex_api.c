 ...
 *
 * Code generation for function '_coder_randomStateGenerator_mex_api'
 *
 */
...
static emlrtRTEInfo p_emlrtRTEI = { 1, 1, "_coder_randomStateGenerator_mex_api",
  "" };
  ...
  const emlrtMsgIdentifier *msgId))[6]
{
  ...
  int32_T iv24[1];
  iv24[0] = 6;
  emlrtCheckBuiltInR2012b(sp, msgId, src, "double", false, 1U, iv24);
  ret = (real_T (*)[6])mxGetData(src);
  emlrtDestroyArray(&src);
  return ret;
}
...
void buildRRTWrapper_api(const mxArray * const prhs[15], const mxArray *plhs[4])
{
  emxArray_real_T *T;
  ...
  buildRRTWrapper(&st, *nInitCartesianB, *nGoalCartesianB, phiInit, omegaInit,
                  *jointLimits, bodyHeight, *U, dt, Dt, &kC, threshold, legNum, *
                  uBDot, *HGAINS, NUM_NODES, T, pathC, pathJ, &success);
...
/* End of code generation (_coder_randomStateGenerator_mex_api.c) */
