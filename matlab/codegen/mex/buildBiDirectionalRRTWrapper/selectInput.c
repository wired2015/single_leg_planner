...
static emlrtRSInfo nb_emlrtRSI = { 30, "selectInput",
...
static emlrtECInfo f_emlrtECI = { -1, 30, 26, "selectInput",
  "/Users/fuji/Dropbox/PhD/matlab/singleLegPlanning/single_leg_planner/matlab/rrt/selectInput.m"
};
                 ...
                 xNew_size[2], emxArray_real_T *transitionArray)
{
  ...
  d1 = (x + 1.0) * 10.0;
  d1 = emlrtNonNegativeCheckFastR2012b(d1, &f_emlrtDCI, sp);
  ixstart = 5 * (int32_T)emlrtIntegerCheckFastR2012b(d1, &e_emlrtDCI, sp);
  for (i5 = 0; i5 < ixstart; i5++) {
    candTransArrays->data[i5] = 0.0;
  }

  emxInit_int32_T(sp, &r2, 1, &k_emlrtRTEI, true);
  ...
  for (i = 0; i < 5; i++) {
    ...
    emlrtSubAssignSizeCheckR2012b(iv9, 2, tmp_size, 2, &e_emlrtECI, sp);
    ixstart = tmp_size[1];
    for (i5 = 0; i5 < ixstart; i5++) {
    ...
    iv10[0] = 1;
    iv10[1] = r2->size[0];
    emlrtSubAssignSizeCheckR2012b(iv10, 2, *(int32_T (*)[2])r3->size, 2,
      &f_emlrtECI, sp);
    ixstart = r3->size[1];
    for (i5 = 0; i5 < ixstart; i5++) {
      candTransArrays->data[i + candTransArrays->size[0] * r2->data[i5]] =
        r3->data[r3->size[0] * i5];
    }
    ...
    x = ((xNear_data[3] - candStates_data[i + 15]) + 3.1415926535897931) /
      6.2831853071795862;
    ...
    if (muDoubleScalarAbs(x - 3.1415926535897931) > 0.39269908169872414) {
      ixstart = 1;
    } else {
    ...
    b_qDot[0] = (-candStates_data[i + 40] * muDoubleScalarSin(candStates_data[i
      + 15]) * ((((kC->l2 - kC->l7) + kC->l5 * muDoubleScalarCos
                  (candStates_data[i + 25] + kC->zeta)) + kC->l3 *
                 muDoubleScalarCos(candStates_data[i + 20])) + kC->l4 *
                muDoubleScalarCos(kC->zeta)) - candStates_data[i + 45] * kC->l3 *
                 muDoubleScalarCos(candStates_data[i + 15]) * muDoubleScalarSin
                 (candStates_data[i + 20])) - candStates_data[i + 50] * kC->l5 *
      muDoubleScalarSin(candStates_data[i + 25] + kC->zeta) * muDoubleScalarCos
      (candStates_data[i + 15]);
    b_qDot[1] = (candStates_data[i + 40] * muDoubleScalarCos(candStates_data[i +
      15]) * ((((kC->l2 - kC->l7) + kC->l5 * muDoubleScalarCos(candStates_data[i
      + 25] + kC->zeta)) + kC->l3 * muDoubleScalarCos(candStates_data[i + 20]))
              + kC->l4 * muDoubleScalarCos(kC->zeta)) - candStates_data[i + 50] *
                 kC->l5 * muDoubleScalarSin(candStates_data[i + 25] + kC->zeta) *
                 muDoubleScalarSin(candStates_data[i + 15])) - candStates_data[i
      + 45] * kC->l3 * muDoubleScalarSin(candStates_data[i + 15]) *
      muDoubleScalarSin(candStates_data[i + 20]);
    b_qDot[2] = -candStates_data[i + 45] * kC->l3 * muDoubleScalarCos
      (candStates_data[i + 20]) - kC->l5 * candStates_data[i + 50] *
      muDoubleScalarCos(kC->zeta + candStates_data[i + 25]);
    for (i5 = 0; i5 < 3; i5++) {
      b_uB[i5] = uB[i5] - uA[i5];
