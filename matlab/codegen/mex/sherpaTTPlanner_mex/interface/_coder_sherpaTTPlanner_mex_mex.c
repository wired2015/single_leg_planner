...
#include "sherpaTTPlanner_mex_terminate.h"
...
emlrtContext emlrtContextGlobal = { true, false, EMLRT_VERSION_INFO, NULL, "sherpaTTPlanner_mex", NULL, false, {2045744189U,2170104910U,2743257031U,4284093946U}, NULL };
...
static void buildBiDirectionalRRTWrapper_mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  sherpaTTPlanner_mexStackData* c_sherpaTTPlanner_mexStackDataL = (sherpaTTPlanner_mexStackData*)mxCalloc(1,sizeof(sherpaTTPlanner_mexStackData));
  ...
  /* Call the function. */
  buildBiDirectionalRRTWrapper_api(c_sherpaTTPlanner_mexStackDataL, inputs, outputs);
  /* Copy over outputs to the caller. */
  for (n = 0; n < nOutputs; ++n) {
    plhs[n] = emlrtReturnArrayR2009a(outputs[n]);
  }
  /* Module finalization. */
  sherpaTTPlanner_mex_terminate();
  mxFree(c_sherpaTTPlanner_mexStackDataL);
}
static void buildRRTWrapper_mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  /* Call the function. */
  buildRRTWrapper_api(c_sherpaTTPlanner_mexStackDataL, inputs, outputs);
  /* Copy over outputs to the caller. */
  for (n = 0; n < nOutputs; ++n) {
    plhs[n] = emlrtReturnArrayR2009a(outputs[n]);
  }
...
void sherpaTTPlanner_mex_atexit_wrapper(void)
{
   sherpaTTPlanner_mex_atexit();
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  mexAtExit(sherpaTTPlanner_mex_atexit_wrapper);
  /* Dispatch the entry-point. */
  method(nlhs, plhs, nrhs-1, prhs+1);
}
/* End of code generation (_coder_sherpaTTPlanner_mex_mex.c) */
