...
void selectInput(const emlrtStack *sp, const real_T xNear[13], const real_T
                 xRand[13], const struct0_T *kC, real_T ankleThreshold, const
                 real_T jointLimits[20], const real_T uBDot[6], int32_T legNum,
                 real_T xNew[13], real_T transitionArray[80])
{
  ...
  for (ixstart = 0; ixstart < 5; ixstart++) {
    ...
    if (angDiff(xNear[3], candStates[15 + ixstart]) > ankleThreshold) {
      aDiff = 1;
    } else {
