                 ...
                 xNew_size[2], emxArray_real_T *transitionArray)
{
  ...
  xStarMin = (dAlphaMax + 1.0) * 10.0;
  xStarMin = emlrtNonNegativeCheckFastR2012b(xStarMin, &f_emlrtDCI, sp);
  ixstart = 9 * (int32_T)emlrtIntegerCheckFastR2012b(xStarMin, &e_emlrtDCI, sp);
  ...
  for (i = 0; i < 9; i++) {
    ...
    dAlphaMax = (((kC->l2 + kC->l3 * muDoubleScalarCos(candStates_data[i + 36]))
                  + kC->l4 * muDoubleScalarCos(kC->zeta)) + kC->l5 *
                 muDoubleScalarCos(kC->zeta + candStates_data[i + 45])) - kC->l7;
    dxStarMax = ((((kC->l2 + kC->l3 * muDoubleScalarCos(xRand[4])) + kC->l4 *
                   muDoubleScalarCos(kC->zeta)) + kC->l5 * muDoubleScalarCos
                  (kC->zeta + xRand[5])) - kC->l7) - dAlphaMax;

    /* angDiff Finds the angular difference between th1 and th2. */
    r = ((candStates_data[i + 27] - xRand[3]) + 3.1415926535897931) /
      6.2831853071795862;
    if (muDoubleScalarAbs(r - muDoubleScalarRound(r)) <= 2.2204460492503131E-16 *
        muDoubleScalarAbs(r)) {
      r = 0.0;
    ...
    /* angDiff Finds the angular difference between th1 and th2. */
    r = ((xNear_data[3] - candStates_data[i + 27]) + 3.1415926535897931) /
      6.2831853071795862;
    if (muDoubleScalarAbs(r - muDoubleScalarRound(r)) <= 2.2204460492503131E-16 *
        muDoubleScalarAbs(r)) {
      r = 0.0;
    ...
    if (muDoubleScalarAbs(r - 3.1415926535897931) > 0.087266462599716474) {
      ixstart = 1;
    } else {
